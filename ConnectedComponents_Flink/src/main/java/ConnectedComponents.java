...
//				vertices.map(new DuplicateValue<Long>());
		...
		DataSet<Tuple2<Long, Long>> verticesWithInitialId =
				vertices.map(vertex -> new Tuple2<Long, Long>(vertex, vertex));
		...
		DataSet<Tuple2<Long, Long>> changes = iteration.getWorkset().join(edges).where(0).equalTo(0).with((Tuple2<Long, Long> vertex, Tuple2<Long, Long> edge) -> new Tuple2<Long, Long>(edge.f1, vertex.f1))
